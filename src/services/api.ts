...
export const dashboardApi = {
  getStats: async (): Promise<DashboardStats> => {
    ...
    const { data: monthlyRevenue } = await supabase
      ...
      .gte('start_date', new Date(new Date().setMonth(new Date().getMonth() - 6)).toISOString())
      .order('start_date', { ascending: true });

    const revenueByMonth = monthlyRevenue?.reduce((acc: any[], contract) => {
      const month = new Date(contract.start_date).toLocaleString('pt-BR', { month: 'short' });
      const existingMonth = acc.find(item => item.month === month);
      
      if (existingMonth) {
        existingMonth.revenue += contract.total;
      } else {
        acc.push({ month, revenue: contract.total });
      }
